      ...
      >
        <CardHeader className="pb-3">
          <div className="flex justify-between items-start">
            <div>
              <CardTitle className="text-xl" id="payment-modal-title">Upgrade Your Plan</CardTitle>
              <CardDescription>Unlock powerful features for your health journey</CardDescription>
            </div>
            <button
              onClick={onClose}
              className="text-foreground/50 hover:text-foreground transition-colors w-8 h-8 flex items-center justify-center rounded-full hover:bg-foreground/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          {/* Progress steps */}
          {step !== 'success' && (
            <div className="flex items-center gap-2 mt-3 text-xs" role="status" aria-live="polite">
        ...
        <CardContent className="space-y-5">
                ...
                {PLAN_ORDER.map((plan) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={selectedPlan === plan && !isCurrent}
                      ...
                      <ul className="space-y-0.5">
                        {p.features.slice(0, 3).map((f, i) => (
                          <li key={i} className="text-xs text-foreground/70 flex items-center gap-1.5">
                            <svg className="w-3 h-3 text-green-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" aria-hidden="true">
          ...
          {step === 'payment-form' && (
            <>
              {/* Payment method tabs */}
              <fieldset className="space-y-3">
                <legend className="text-sm font-medium text-foreground">Select Payment Method</legend>
                <div className="flex gap-2 p-1 bg-muted rounded-lg">
                  {(['card', 'upi', 'mock'] as const).map((m) => (
                    <button
                      key={m}
                      onClick={() => setPaymentMethod(m)}
                      className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                        paymentMethod === m
              ...
              {paymentMethod === 'card' && (
                <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); handlePay(); }}>
                  <div>
                    <label htmlFor="card-number" className="text-xs font-medium text-foreground/70 block mb-1">Card Number</label>
                    <input
                      id="card-number"
                      type="text"
                      inputMode="numeric"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label htmlFor="card-expiry" className="text-xs font-medium text-foreground/70 block mb-1">Expiry</label>
                      <input
                        id="card-expiry"
                        type="text"
                        inputMode="numeric"
                    ...
                    <div>
                      <label htmlFor="card-cvc" className="text-xs font-medium text-foreground/70 block mb-1">CVC</label>
                      <input
                        id="card-cvc"
                        type="text"
                        inputMode="numeric"
                        ...
                        placeholder="123"
                        aria-label="3 or 4 digit security code"
                        className="w-full px-3 py-2 border border-border rounded-lg bg-input text-foreground text-sm font-mono focus:outline-none focus:ring-2 focus:ring-ring"
                      />
                    </div>
                  </div>
                  <div>
                    <label htmlFor="card-name" className="text-xs font-medium text-foreground/70 block mb-1">Name on Card</label>
                    <input
                      id="card-name"
                      type="text"
                      value={cardName}
                      ...
                      aria-label="Full name as shown on card"
                  ...
                  <div className="flex items-center gap-2 text-xs text-foreground/50">
                    <svg className="w-4 h-4 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
              ...
              {paymentMethod === 'upi' && (
                <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); handlePay(); }}>
                  <div>
                    <label htmlFor="upi-id" className="text-xs font-medium text-foreground/70 block mb-1">UPI ID</label>
                    <input
                      id="upi-id"
                      type="text"
                      value={upiId}
                      ...
                      aria-label="UPI ID (e.g. name@upi)"
                      className="w-full px-3 py-2 border border-border rounded-lg bg-input text-foreground text-sm font-mono focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                  </div>
                  <div className="bg-muted/40 rounded-lg p-3 flex items-center gap-3" role="img" aria-label="QR code for payment via UPI">
                    <div className="w-12 h-12 bg-card border border-border rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-6 h-6 text-foreground/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
